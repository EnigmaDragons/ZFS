...
namespace MonoDragons.ZFS.Characters
{
    public sealed class CharacterBody : IAutomaton, IVisual
    {
        ...
        public void Update(TimeSpan delta)
        {
            ...
            {
                double speedModifier = CurrentData.CurrentCharacter.Team != Team.Friendly && !CurrentData.FriendlyPerception[CurrentTile.Position]
                    ? UnseenEnemySpeed
                    : CurrentData.CurrentCharacter.Team == Team.Friendly
                        ? RunSpeed : EnemySpeed;

                var target = Path.First();
                var targetLocation = CurrentData.Map[target].Transform.Location;
                ...
                if (CurrentTileLocation.X == targetLocation.X && CurrentTileLocation.Y == targetLocation.Y)
                {
                    ...
                    Event.Publish(new Moved { Character = CurrentData.CurrentCharacter, Position = target });
                }
            }
        }
