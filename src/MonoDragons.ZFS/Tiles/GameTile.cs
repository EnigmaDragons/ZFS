...
namespace MonoDragons.ZFS.Tiles
{
    public class GameTile
    {
        ...
        public bool IsWalkable => Details.All(x => !x.IsBlocking) && CurrentData.LivingCharacters.All(x => x.CurrentTile != this);
        ...
        public void Draw(int layer, Transform2 parentTransform, Color tint)
        {
            ...
            if (layer.Equals(FogLayer) && !CurrentlyFriendlyVisible)
                ...
                Details.Where(x => x.ZIndex == layer)
                    .Where(x => x.IsVisible)
                    .ForEach(x => World.Draw(x.Texture, (parentTransform + Transform).ToRectangle(), x.SourceRect, tint));
        }
    }
}
